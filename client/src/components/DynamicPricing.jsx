      ...
      <div className="mt-6 p-4 bg-blue-50 rounded-lg">
        <div className="flex items-center justify-between text-sm">
          <div className="text-blue-800">
            <div className="font-semibold">Fare Estimate</div>
            <div className="text-xs opacity-90">Price may vary based on traffic and demand</div>
          </div>
          <div className="text-blue-600 text-xs">
            <div>Base fare + distance ({estimatedDistance.toFixed(1)}km)</div>
            {priorityPickup && <div>+ Priority pickup (€5)</div>}
            {new Date().getHours() >= 22 || new Date().getHours() <= 6 ? (
              <div>+ Night surcharge (20%)</div>
            ) : null}
          </div>
        </div>
      </div>
